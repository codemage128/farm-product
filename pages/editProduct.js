...
function EditProduct(props) {
    ...
    const INITIAL_PRODUCT = {
        ...
        media: props.product.mediaUrl,
        description: props.product.description,
        location: props.product.location,
        deliveryArea: props.product.deliveryArea,
        inventoryQuantity: props.product.inventoryQuantity,
        includedProducts: props.farmProduct,
        deliveryMethods: props.product.deliveryMethods,
    };

    const [product, setProduct] = React.useState(INITIAL_PRODUCT);
    const [preCenter, setPreCenter] = React.useState(INIT_PRECENTER);
    const [zoom, setZoom] = React.useState(5);
    const [mediaPreview, setMediaPreview] = React.useState(props.product.mediaUrl);
    ...
    function addProduct(event) {
        ...
        if (productSelect) {
            ...
            if (!isExist) {
                let product = data.farmProducts.find((p) => p._id === productSelect);
    ...
    return (
        <Container>
            ...
            <Message>
                <p>
                    Update products with one or more of your Farm Produce items, which will appear on the shop
            </p>
            </Message>
            ...
            <Segment>
                ...
                >
                    ...
                    <Select
                        ...
                        options={props.farmProducts.map((product) => {
                            return {
                                key: product._id,
                                text: product.name,
                                value: product._id,
                    ...
                    {product.deliveryMethods.map((d) => {
                        if (!d.text) {
                            d.text = DELIVERYMETHODS.find(delivery => delivery.type === d.type).text;
                        }
                        ...
                        return (
                            <Form.Group widths="equal" key={d.type}>
                                ...
                                <Form.Field
                                    ...
                                    readOnly={d.type === "customer_pickup"}
                                />
                                <Form.Field
                                    control={Input}
                                    name="d_unit"
...
EditProduct.getInitialProps = async (ctx) => {
    ...
    const url = `${baseUrl}/api/product`;
    let type = "edit"
    const payload = { headers: { Authorization: token }, params: { _id, type } };
    const response = await axios.get(url, payload);

    const owner = response.data.product.owner;

    const owner2 = jwt.verify(
        token,
        process.env.JWT_SECRET
    );

    if (owner === owner2.userId) {
